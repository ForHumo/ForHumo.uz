...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    const result = await prisma.$transaction(async tx => {
        ...
        await tx.walletTransaction.create({ data: { walletId: wallet.id, type: "TRANSFER_OUT", amount: pay, currency: wCur, balanceAfter: roundMoney(Number(after?.balance ?? 0), wCur), description: "Turnir mukofot fondi (escrow)", ref: `tourfund:${id}:${crypto.randomUUID()}` } });
