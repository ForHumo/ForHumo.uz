// Suhbatni ovozsizlantirish/qaytarish (per-user).
//   POST /api/nexus/messages/[convId]/mute-conv  { mute: true|false }
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    await prisma.nexusConversation.update({
        where: { id },
        data: isUser1
            ? { mutedByUser1: mute ? new Date() : null }
            : { mutedByUser2: mute ? new Date() : null },
    });
