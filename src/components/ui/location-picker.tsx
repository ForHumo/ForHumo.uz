...
export function LocationPicker({ value, onChange, accent = "blue" }: Props) {
    ...
    // Modal yuzasi — Market (yashil) rejimida global ko'k tema o'rniga yashil/neytral
    const surface = mapOnly
        ? "bg-white/95 dark:bg-[#08160c]/97 border border-green-100 dark:border-green-900/40"
        : "bg-card border border-border/60";
    const panelBorder = mapOnly ? "border-green-100 dark:border-white/[0.07]" : "border-border/60";
    const headText = mapOnly ? "text-gray-900 dark:text-white" : "text-foreground";
    const subText = mapOnly ? "text-gray-500 dark:text-white/40" : "text-muted-foreground";
    const toggleWrap = mapOnly ? "bg-gray-100 dark:bg-white/[0.06]" : "bg-muted/60";
    const toggleOn = mapOnly ? "bg-green-500 text-white shadow-sm" : "bg-card text-foreground shadow-sm";
    const toggleOff = mapOnly ? "text-gray-500 dark:text-white/40 hover:text-gray-800 dark:hover:text-white" : "text-muted-foreground hover:text-foreground";
    const ghostBtn = mapOnly
        ? "border-gray-200 dark:border-white/[0.08] hover:bg-gray-50 dark:hover:bg-white/[0.04] text-gray-600 dark:text-white/50 hover:text-gray-900 dark:hover:text-white"
        : "border-border/60 hover:bg-muted/50 text-muted-foreground hover:text-foreground";
    const fieldSurface = mapOnly ? "bg-gray-50 dark:bg-white/[0.04] border-gray-200 dark:border-white/[0.08]" : "bg-muted/40 border-border/60";
                ...
                {mapOpen && (
                    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
                        <motion.div
                            ...
                            className={`w-full sm:max-w-lg ${surface} sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col`}
                            style={{ maxHeight: "90vh" }}
                        >
                            {/* Modal header */}
                            <div className={`flex items-center justify-between px-4 py-3 border-b ${panelBorder} shrink-0 gap-3`}>
                                <div className="min-w-0">
                                    <h3 className={`text-sm font-bold ${headText} truncate`}>{t("location_pick_title")}</h3>
                                    <p className={`text-xs ${subText}`}>{t("location_pick_hint")}</p>
                                </div>

                                {/* Street / Satellite toggle */}
                                <div className={`flex items-center ${toggleWrap} rounded-lg p-0.5 shrink-0`}>
                                    <button
                                        type="button"
                                        onClick={() => setMapMode("street")}
                                        className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-all whitespace-nowrap ${
                                            mapMode === "street" ? toggleOn : toggleOff
                                        }`}
                                    >
                                        {t("location_street")}
                                    </button>
                                    <button
                                        ...
                                        className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-all whitespace-nowrap ${
                                            mapMode === "satellite" ? toggleOn : toggleOff
                                        }`}
                                    >
                            ...
                            {/* Bottom panel */}
                            <div className={`px-4 py-3 border-t ${panelBorder} space-y-3 shrink-0`}>
                                {/* Geo detect button */}
                                <button
                                    type="button"
                                    onClick={detectGeo}
                                    disabled={geoLoading}
                                    className={`w-full flex items-center justify-center gap-2 h-9 rounded-xl border text-xs font-medium ${ghostBtn} transition-all disabled:opacity-60`}
                                >
                                    {geoLoading ? (
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    {mapOnly ? (
                                        <div className={`flex-1 ${fieldSurface} border rounded-lg px-3 py-2 text-sm ${headText} min-h-[2.5rem] flex items-center`}>
                                            {pendingAddress || <span className={subText}>Xaritani bosib manzilni tanlang</span>}
                                        </div>
                                    ) : (
                                        <textarea
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className={`flex-1 h-10 rounded-xl border text-sm font-medium ${ghostBtn} transition-all`}
                                    >
                                        {tCommon("cancel")}
                                    </button>
                                    <button
