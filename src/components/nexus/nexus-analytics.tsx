        ...
        <div className="h-full overflow-y-auto text-[var(--nx-text)]" style={{ background: "var(--nx-bg)" }}>
            <header className="sticky top-0 z-20 flex items-center gap-3 px-3 h-14 backdrop-blur-xl"
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-black text-[var(--nx-text)]">Ijodkor analitikasi</span>
                </div>
            </header>
                                ...
                                {data.recentTips.map(t => (
                                    <div key={t.id} className="flex items-center gap-3 p-2.5 rounded-2xl" style={{ background: "var(--nx-surface)", border: "1px solid rgba(245,158,11,0.16)" }}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-bold text-[var(--nx-text)] truncate">{t.donor?.name || t.donor?.username || "Foydalanuvchi"}</p>
                                            {t.message && <p className="text-[11px] truncate" style={{ color: "var(--nx-text-3)" }}>{t.message}</p>}
                                        </div>
...
function EarnCell({ icon: Icon, color, label, value, currency }: { icon: typeof Gift; color: string; label: string; value: number; currency: Currency }) {
    return (
        <div className="rounded-xl px-2.5 py-2" style={{ background: "var(--nx-surface-2)" }}>
