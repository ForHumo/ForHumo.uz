            ...
            {loading ? (
                ...
                <div className="px-4 py-4 pb-28 max-w-2xl mx-auto">
                    ...
                    <div className="rounded-3xl p-5 mb-4" style={{ background: "linear-gradient(135deg, rgba(43,62,232,0.18), rgba(0,206,200,0.12))", border: "1px solid rgba(0,206,200,0.25)" }}>
                        ...
                        <p className="text-4xl font-black mt-1" style={{ color: "#fff" }}>{fmtN(data.earnings.total)} <span className="text-2xl" style={{ color: "#00CEC8" }}>Ƶ</span></p>
                        {data.earnings.subMonthly > 0 && (
                            <p className="text-xs mt-1.5 flex items-center gap-1" style={{ color: "rgba(196,181,253,0.95)" }}>
                                <Star className="w-3 h-3" style={{ color: "#8B5CF6" }} />+ {fmtN(data.earnings.subMonthly)} Ƶ/oy obunadan (takrorlanuvchi)
                            </p>
                        )}
                        <div className="grid grid-cols-3 gap-2 mt-4">
                            <EarnCell icon={Gift} color="#F59E0B" label="Tip" value={data.earnings.tips} />
                            <EarnCell icon={ShoppingBag} color="#10B981" label="Video sotuv" value={data.earnings.videoSales} />
                            <EarnCell icon={Star} color="#8B5CF6" label="Obuna/oy" value={data.earnings.subMonthly} />
                        </div>
                    </div>
                                ...
                                {data.recentTips.map(t => (
                                    <div key={t.id} className="flex items-center gap-3 p-2.5 rounded-2xl" style={{ background: "rgba(11,18,40,0.55)", border: "1px solid rgba(245,158,11,0.16)" }}>
                                        ...
                                        <div className="text-right flex-shrink-0">
                                            <p className="text-sm font-black" style={{ color: "#F59E0B" }}>{t.amountZij} Ƶ</p>
...
function EarnCell({ icon: Icon, color, label, value }: { icon: typeof Gift; color: string; label: string; value: number }) {
    return (
        <div className="rounded-xl px-2.5 py-2" style={{ background: "rgba(5,8,24,0.45)" }}>
            ...
            <p className="text-base font-black text-white mt-0.5">{fmtN(value)} <span className="text-xs" style={{ color }}>Ƶ</span></p>
        </div>
    );
}
