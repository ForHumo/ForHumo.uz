...
interface VidItem { id: string; title: string; thumbUrl: string | null; videoUrl: string; durationSec: number; orientation: string; views: number; likeCount: number; priceZij: number; locked: boolean; author: { name: string | null; username: string | null; image: string | null } | null }
                                ...
                                {videos.map(v => (
                                    <button key={v.id} onClick={() => openVid(v)} className="text-left group">
                                        <div className="relative aspect-video rounded-xl overflow-hidden mb-1.5" style={{ background: "rgba(11,18,40,0.7)", border: "1px solid rgba(43,62,232,0.18)" }}>
                                            ...
                                            {v.locked && <span className="absolute top-1.5 left-1.5 flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-black text-white" style={{ background: "rgba(43,62,232,0.9)" }}><Lock className="w-2.5 h-2.5" />{fmtN(v.priceZij)} Ƶ</span>}
