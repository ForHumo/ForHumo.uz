...
interface KinoVid {
    ...
    orientation: "HORIZONTAL" | "VERTICAL"; locked: boolean; priceZij: number;
                        ...
                        {kino.map(v => (
                            <div key={v.id} role="button" tabIndex={0}
                                ...
                                <div className="relative aspect-video rounded-xl overflow-hidden mb-2" style={{ border: "1px solid rgba(43,62,232,0.18)", background: "rgba(43,62,232,0.08)" }}>
                                    ...
                                    {v.priceZij > 0 && <span className="absolute top-2 left-2 px-1.5 py-0.5 rounded text-[10px] font-black text-white" style={{ background: "linear-gradient(135deg,#2B3EE8,#00CEC8)" }}>{v.priceZij} Ƶ</span>}
