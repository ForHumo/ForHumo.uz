            ...
            <div className="fixed inset-x-0 bottom-0 z-[60] flex flex-col rounded-t-3xl overflow-hidden md:inset-x-auto md:inset-y-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-[460px] md:max-h-[85vh] md:rounded-3xl"
                ...
                <div className="flex-1 overflow-y-auto px-4 py-3" style={{ scrollbarWidth: "none" }}>
                    {loading ? (
                        ...
                        <div className="flex flex-col gap-2">
                            {filtered.map(u => (
                                <div key={u.username ?? u.name ?? ""} className="flex items-center gap-3 p-3 rounded-2xl" style={{ background: "var(--nx-surface)", border: "1px solid rgb(var(--nx-accent-rgb) / 0.16)" }}>
                                    <Link href={u.username ? `/nexus/u/${u.username}` : "/nexus"} onClick={() => setSubsOpen(false)}
                                        className="flex items-center gap-3 flex-1 min-w-0">
                                        <img src={avatarOf(u)} alt="" className="w-10 h-10 rounded-xl object-cover bg-white flex-shrink-0" style={{ border: "1px solid rgb(var(--nx-accent-rgb) / 0.25)" }} />
...
function UserSkeleton() {
    return (
        <div className="flex items-center gap-3 p-3 rounded-2xl animate-pulse" style={{ background: "var(--nx-surface)", border: "1px solid rgb(var(--nx-accent-rgb) / 0.12)" }}>
            <div className="w-10 h-10 rounded-xl flex-shrink-0" style={{ background: "rgb(var(--nx-accent-rgb) / 0.15)" }} />
            <div className="flex-1 space-y-1.5">
                <div className="h-2.5 rounded" style={{ background: "rgb(var(--nx-accent-rgb) / 0.15)", width: "50%" }} />
                <div className="h-2 rounded" style={{ background: "rgb(var(--nx-accent-rgb) / 0.10)", width: "30%" }} />
            </div>
            <div className="h-7 w-20 rounded-lg flex-shrink-0" style={{ background: "rgb(var(--nx-accent-rgb) / 0.10)" }} />
        </div>
    );
}
