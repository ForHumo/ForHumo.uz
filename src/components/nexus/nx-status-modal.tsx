...
// Maxsus status tanlash modali — emoji + matn + amal muddati.

import { useState } from "react";
import { X, Loader2, Trash2 } from "lucide-react";

const PRESETS: Array<{ emoji: string; text: string; mins?: number }> = [
    { emoji: "💻", text: "Ishda", mins: 8 * 60 },
    { emoji: "🎮", text: "O'yinda", mins: 60 },
    { emoji: "🎧", text: "Musiqa tinglayapman" },
    { emoji: "📚", text: "O'qiyapman", mins: 60 },
    { emoji: "🏃", text: "Sport zalida", mins: 90 },
    { emoji: "🍽️", text: "Ovqatlanmoqda", mins: 30 },
    { emoji: "😴", text: "Uxlayapman" },
    { emoji: "🚗", text: "Yo'ldaman" },
    { emoji: "🎬", text: "Film ko'ryapman" },
    { emoji: "🤒", text: "Kasal", mins: 24 * 60 },
    { emoji: "🌴", text: "Ta'tilda", mins: 7 * 24 * 60 },
    { emoji: "📴", text: "Bandman" },
];
...
export function NxStatusModal({ initialEmoji, initialText, onClose, onSaved }: Props) {
    ...
    async function save() {
        if (!emoji && !text.trim()) { setErr("Emoji yoki matn tanlang"); return; }
        setBusy(true);
        setErr(null);
        try {
            const r = await fetch("/api/user/status", {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ emoji: emoji || undefined, text: text.trim() || undefined, expiresInMinutes: mins ?? 0 }),
            });
            const d = await r.json().catch(() => ({}));
            if (r.ok) { onSaved(emoji || null, text.trim() || null); onClose(); }
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4"
            ...
            <div onClick={e => e.stopPropagation()}
                ...
                <div className="p-4 space-y-3">
                    <div className="flex gap-2">
                        <input value={emoji} onChange={e => setEmoji(e.target.value.slice(0, 4))}
                            placeholder="🎮" className="w-16 h-11 text-center text-2xl rounded-lg bg-transparent focus:outline-none"
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                            {PRESETS.map((p, i) => (
                                <button key={i} onClick={() => {
                                    setEmoji(p.emoji); setText(p.text);
                                    if (typeof p.mins === "number") setMins(p.mins);
                                }}
                                    className="flex items-center gap-2 px-2.5 py-2 rounded-lg text-left transition hover:bg-white/[0.05]"
                                    style={{ background: "rgba(43,62,232,0.06)", border: "1px solid rgba(43,62,232,0.15)" }}>
                                    <span className="text-lg flex-shrink-0">{p.emoji}</span>
                                    <span className="text-xs truncate" style={{ color: "rgba(220,230,255,0.90)" }}>{p.text}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="p-4 border-t flex gap-2" style={{ borderColor: "rgba(43,62,232,0.20)" }}>
                    ...
                    <button onClick={save} disabled={busy || (!emoji && !text.trim())}
