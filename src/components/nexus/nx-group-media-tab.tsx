    ...
    return (
        <>
            ...
            <div className="fixed inset-x-0 bottom-0 z-[321] flex max-h-[85vh] flex-col overflow-hidden rounded-t-3xl md:inset-y-0 md:right-0 md:inset-x-auto md:max-h-full md:w-[440px] md:rounded-none md:rounded-l-3xl"
                ...
                {/* Tabs */}
                <div className="flex gap-1 px-3 py-2 flex-shrink-0" style={{ borderBottom: "1px solid rgb(var(--nx-accent-rgb) / 0.14)" }}>
                    {TABS.map(t => (
                        <button key={t.key}
                            ...
                            {items.map((it, i) => (
                                <a key={i} href={it.url} target="_blank" rel="noopener"
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {it.size && <p className="text-[11px]" style={{ color: "rgb(var(--nx-text-2-rgb)/0.7)" }}>{formatSize(it.size)}</p>}
                                    </div>
                                    <ExternalLink className="w-4 h-4 flex-shrink-0" style={{ color: "rgb(var(--nx-text-2-rgb)/0.6)" }} />
                                </a>
                            ))}
                        </div>
                    ) : (
