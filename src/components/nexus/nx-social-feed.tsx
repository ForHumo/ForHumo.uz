...
interface Post {
    ...
    marketProductId: string | null; shareCount: number; createdAt: string;
            ...
            <div className="mx-4 mb-4 p-4 rounded-2xl"
                ...
                {(postText || media.length > 0) && (
                    <div className="mt-3 flex items-center justify-between pl-12">
                        ...
                        <button onClick={submitPost} disabled={sending || uploading}
                            className="px-4 py-1.5 rounded-xl text-xs font-black text-white transition-all duration-150 active:scale-95"
                            style={{ background: "linear-gradient(135deg,#2B3EE8,#00CEC8)", opacity: sending ? 0.7 : 1 }}>
