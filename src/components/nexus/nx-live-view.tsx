    ...
    const load = useCallback(async (silent = false) => {
        ...
        try {
            ...
            const [l, u, e] = await Promise.all([
                fetch(`/api/nexus/live?status=live&limit=24${catQ}`).then(r => r.json()),
                fetch(`/api/nexus/live?status=upcoming&limit=12${catQ}`).then(r => r.json()),
                fetch(`/api/nexus/live?status=ended&limit=12${catQ}`).then(r => r.json()),
            ]);
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-1 duration-250 pb-32">
            ...
            <div className="mx-4 mt-4 mb-3 p-5 rounded-2xl relative overflow-hidden" style={{ background: "rgba(11,18,40,0.60)", border: "1px solid rgba(239,68,68,0.22)" }}>
                ...
                <p className="text-xs mb-4 relative" style={{ color: "rgba(140,160,210,0.75)" }}>Real vaqtda — chat va ko&apos;ruvchilar jonli</p>
            ...
            {loading ? (
                <div className="flex justify-center py-16"><Loader2 className="w-7 h-7 animate-spin" style={{ color: "#EF4444" }} /></div>
            ) : (
                <>
                    {/* Hozir jonli */}
                    <Section title="Hozir Jonli" accent="#EF4444">
                        {live.length === 0 ? (
                            <Empty text="Hozir hech kim jonli emas — birinchi bo'ling!" />
                        ) : (
...
function StreamCard({ s, onOpen, badge, meta, dim }: {
    ...
    return (
        <div role="button" tabIndex={0} onClick={onOpen} onKeyDown={e => e.key === "Enter" && onOpen()}
            ...
            <div className="relative aspect-video rounded-xl overflow-hidden mb-2 flex items-center justify-center"
                ...
                {/* Streamer avatari markazda — video transport keyin */}
                ...
                {s.category && <span className="absolute top-2 right-2 px-1.5 py-0.5 rounded text-[10px] font-bold text-white" style={{ background: "rgba(5,8,24,0.75)" }}>#{s.category}</span>}
