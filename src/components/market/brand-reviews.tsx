...
import {
    Star, MessageSquare, Loader2, Send, Lock, CheckCircle2, AlertCircle,
} from "lucide-react";
...
export function BrandReviews({ slug }: { slug: string }) {
    const [reviews, setReviews] = useState<BReview[]>([]);
    ...
    function load() {
        fetch(`/api/market/brands/${slug}/reviews`)
            .then(r => r.json())
            .then(d => { setReviews(d.reviews ?? []); setAvg(d.avgRating ?? 0); setCanReview(d.canReview ?? false); setAlready(d.alreadyReviewed ?? false); })
    ...
    async function submit(e: React.FormEvent) {
        ...
        try {
            const res = await fetch(`/api/market/brands/${slug}/reviews`, {
        ...
        <section className="pb-10">
            <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-gray-900 dark:text-white text-lg">
                        Brend sharhlari {!loading && `(${reviews.length})`}
                    </h2>
                    {avg > 0 && (
                        <span className="flex items-center gap-1 text-sm font-bold text-amber-500 ml-1">
                            <Star size={14} className="fill-amber-400 text-amber-400" /> {avg}
                        </span>
                    )}
                </div>
                {canReview && !showForm && (
            ...
            {!loading && !canReview && !already && (
                <div className="flex items-center gap-2 bg-gray-50/80 dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.06] rounded-2xl px-4 py-3 mb-5">
                    ...
                    <p className="text-xs text-gray-500 dark:text-white/35">Sharh uchun avval shu brenddan harid qiling</p>
                </div>
            )}
            {already && (
                <div className="flex items-center gap-2 bg-green-50/80 dark:bg-green-900/10 border border-green-200/60 dark:border-green-800/20 rounded-2xl px-4 py-3 mb-5">
                    <CheckCircle2 size={14} className="text-green-500" />
                    <p className="text-xs text-green-700 dark:text-green-400">Siz bu brendga sharh qoldirgansiz</p>
                </div>
            )}
                    ...
                    <motion.form onSubmit={submit}
                        ...
                        <div className="bg-white/70 dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.06] rounded-2xl p-5">
                            ...
                            <textarea value={text} onChange={e => setText(e.target.value)} rows={3} maxLength={500}
                                placeholder="Brend haqida fikringiz..."
            ...
            {loading ? (
                ...
                <p className="text-sm text-gray-400 dark:text-white/25 py-6 text-center">Hali brend sharhlari yo&apos;q</p>
            ) : (
                            ...
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-0.5">
                                        {[1,2,3,4,5].map(s => (
                                            <Star key={s} size={11} className={s <= r.rating ? "text-amber-400 fill-amber-400" : "text-gray-200 dark:text-white/10"} />
                                        ))}
                                    </div>
                                </div>
                                <span className="text-xs text-gray-300 dark:text-white/20">{new Date(r.createdAt).toLocaleDateString("uz-UZ")}</span>
                            </div>
