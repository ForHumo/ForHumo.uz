...
**Nexus:** `NexusPost` (+`marketProductId` — shoppable), `NexusLike`, `NexusSave`, `NexusComment` (`parentId`), `NexusFollow`, `NexusNotification`
...
| **Nexus** | `/nexus` (SPA), `/nexus/u/[username]`, `/nexus/tag/[tag]` | `api/nexus/*` (feed/profile/follows/search/discover/notifications/report) | 🟡 Core real | Feed + shoppable + **profil** + **qidiruv/kashf** + **bildirishnoma** (like/izoh/follow/javob + qo'ng'iroq badge) + shikoyat real. Qolgan ~50 komponent (stories/video/live/media/DM) mock |
