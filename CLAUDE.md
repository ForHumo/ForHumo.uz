...
**Nexus:** `NexusPost` (+`marketProductId` — shoppable), `NexusLike`, `NexusSave`, `NexusComment` (`parentId`), `NexusFollow`, `NexusNotification`, `NexusStory` (24 soat), `NexusStoryView`
...
| **Nexus** | `/nexus` (SPA), `/nexus/u/[username]`, `/nexus/tag/[tag]` | `api/nexus/*` (feed/profile/follows/search/discover/notifications/stories/report) | 🟡 Core real | Feed + shoppable + **profil** + **qidiruv/kashf** + **bildirishnoma** + **stories** (24 soat, halqa, segmentli viewer) + shikoyat real. Qolgan ~48 komponent (video/live/media/DM) mock |
