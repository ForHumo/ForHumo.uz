...
**Nexus:** `NexusPost` (+`marketProductId` — shoppable), `NexusLike`, `NexusSave`, `NexusComment` (`parentId`), `NexusFollow`, `NexusNotification`, `NexusStory` (24 soat), `NexusStoryView`, `NexusConversation` (1:1 DM), `NexusMessage`, `NexusVideo` (LONG/SHORT), `NexusVideoView`, `NexusVideoLike`, `NexusVideoComment`
...
| **Nexus** | `/nexus` (SPA), `/nexus/u/[username]`, `/nexus/tag/[tag]` | `api/nexus/*` (…stories/messages/videos/report) | 🟡 Core real | Feed + profil + qidiruv/kashf + bildirishnoma + stories + DM + **Video** (uzun: real MP4 yuklash/player/ko'rish/like/izoh/obuna) real. Qolgan ~44 komponent (live/media + Shorts vertical) mock |
