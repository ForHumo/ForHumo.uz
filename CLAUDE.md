...
**Nexus:** `NexusPost` (+`marketProductId` — shoppable), `NexusLike`, `NexusSave`, `NexusComment` (`parentId`), `NexusFollow`, `NexusNotification`, `NexusStory` (24 soat), `NexusStoryView`, `NexusConversation` (1:1 DM), `NexusMessage`
...
| **Nexus** | `/nexus` (SPA), `/nexus/u/[username]`, `/nexus/tag/[tag]` | `api/nexus/*` (feed/profile/follows/search/discover/notifications/stories/messages/report) | 🟡 Core real | Feed + shoppable + **profil** + **qidiruv/kashf** + **bildirishnoma** + **stories** + **DM** (1:1, polling, unread badge) + shikoyat real. Qolgan ~46 komponent (video/live/media) mock |
