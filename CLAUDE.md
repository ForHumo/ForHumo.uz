...
| **Nexus** | `/nexus` (SPA), `/nexus/u/[username]`, `/nexus/tag/[tag]` | `api/nexus/*` (feed/profile/follows/search/discover/report) | 🟡 Core real | Feed + shoppable + **profil** + **qidiruv/kashf** (odamlar/post/hashtag, trend, tavsiya) + shikoyat real. Qolgan ~50 komponent (stories/video/live/media/DM) mock |
